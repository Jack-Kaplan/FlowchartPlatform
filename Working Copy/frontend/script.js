...
function updateAttributes() {
    ...
    // Clear existing headers except for "Element"
    ...
    for (let i = 0; i < tbody.rows.length; i++) {
        ...
        while (row.cells.length < attributes.length + 1) {
...
function removeElement() {
    const table = document.getElementById('dataTable');
    const tbody = table.getElementsByTagName('tbody')[0];
    const selectedRows = Array.from(tbody.getElementsByTagName('tr'))
        .filter(row => row.querySelector('input:checked'));

    selectedRows.forEach(row => tbody.removeChild(row));
}
...
function getDataFromTable() {
    ...
    for (let i = 1; i < table.rows.length; i++) {
        ...
        const rowData = {
            Element: row.cells[0].querySelector('input').value,
            attributes: {}
        };
        ...
        for (let j = 1; j < row.cells.length; j++) {
            ...
            rowData.attributes[headers[j]] = value;
        }
